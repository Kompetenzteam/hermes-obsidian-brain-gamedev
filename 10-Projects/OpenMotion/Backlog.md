...
- [x] **M6.6 Prototyp V2 ✅** — EnvironmentBuilder (Boden 3000×3000, pastelliger Himmel, Ambient, Sonne), BuildingVisualizer (CityGrowth-Gebäude als 3D-Quader, warme Farben), dichtere Stadtkarte (10 Straßen/3 Schienen/2 Gehwege/17 Haltestellen, 3×3-Blöcke), **Fix: CityGrowth wächst auf Referenzkarten-Infrastruktur** (Gebäude bei 1000,1000 statt Weltursprung) — **217 Tests grün**
- [x] **M6.7 Performance-Welle ✅** — **Root-Cause gefunden**: CityGrowth `IsOccupied()` O(N²) (309→3020 ms/Tick) → **Spatial-Hash-Fix: ~44 ms/Tick (Release, flach), determinismus-identisch**; GPU-Instancing (MultiMesh, Draw Calls ~1 statt 2×N); Refresh-Throttling (Vehicles 15 Hz/Buildings 2 Hz); max_fps 60 + MSAA aus; PerfMonitor (FPS/DrawCalls-Logging); Threading-Analyse (Hash ist nicht der Bottleneck) — **219 Tests grün**; Prototyp V3-Export läuft
