...
### scripts/BuildingVisualizer.cs (M6.6/M6.7, GPU-Instancing + inkrementell)
- **`BuildingVisualizer`** — `public partial class BuildingVisualizer : Node3D` — Gebäude-Visualisierung (M6.6, GDD 4; M6.7-Perf-Umbau + Perf-Welle 2): rendert die CityGrowth-Gebäude als Instanzen EINES MultiMeshInstance3D (Einheits-BoxMesh, 1x1x1, pro Instanz skaliert/positioniert) — **Draw Calls ~1 unabhängig von N** (vorher 2 MeshInstance3D je Gebäude ⇒ 1200+ Draw Calls bei 600+ Gebäuden, User-Feedback „extremes Ruckeln"); **inkrementelles Update** (2026-08-09): wächst der Bestand, werden NUR die neuen Instanzen angehängt (`SetInstanceTransform`/`SetInstanceColor` für `[renderedCount, count)` + `VisibleInstanceCount` erhöhen) — alte Instanzen bleiben unberührt, kein Voll-Rebuild/GPU-Upload je Wachstum; sim X/Y → Godot X/Z (Y nach oben, Konvention wie MapRenderer/VehicleVisualizer); Fix32→float ausschliesslich in der Render-Schicht (Sim bleibt Fix32-exakt); reines Lesen, kein Sim-Eingriff.
...
- **Höhen-/Abmessungs-Konstanten** — Höhenbereiche (Residential 6–10, Commercial 12–20, Industrial 8–14 m), Grundflächen (8/12/14 m), `WindowBandVMin 0.305`/`WindowBandVMax 0.855` (UV-Band), `SyncIntervalMsec = 500` (Rate-Limit, max. 2 Updates/s), `ColorVariantsPerType = 8`.
- **Puffer-Konstanten (Perf-Welle 2)** — `InitialInstanceCapacity = 4096` (Start-Kapazität, 256 KiB = 4096 × 16 floats × 4 Byte; deckt den Frühverlauf ohne Rebuild ab); Puffer-Modell: `InstanceCount` = Kapazität (Chunk-Wachstum per Verdopplung), `VisibleInstanceCount` = geschriebene = sichtbare Instanzen.
- **`BuildingNodeCount`** — `public int BuildingNodeCount => _renderedCount` — Anzahl der aktuell instanziierten Gebäude (= MultiMesh.VisibleInstanceCount, Headless-Nachweis; Name aus Kompatibilität zu SimulationRunner erhalten — es werden keine Gebäude-Knoten mehr erzeugt).
- **`Initialize(CityGrowthSystem)`** — `public void Initialize(CityGrowthSystem growth)` — Registriert den Wachstums-Kern (nur Lesen, ArgumentNullException-Guard), legt den MultiMeshInstance3D „Buildings" einmalig an (CreateMultiMesh, InstanceCount zunächst 0), Start-Log, dann Refresh().
- **`Refresh()`** — `public void Refresh()` — M6.6/M6.7-Hook (vom SimulationRunner gedrosselt alle 15 Sim-Ticks gerufen, intern zusätzlich zeitbasiert auf max. 2 Updates/s limitiert): aktualisiert den Instanz-Puffer NUR bei Änderung der Gebäude-Anzahl (Gebäude unveränderlich — Anzahl-Änderung = vollständiges Dirty-Signal; kein Clear+Neuaufbau je Tick). **INKREMENTELL (Perf-Welle 2):** Wachstum (Normalfall) ⇒ `AppendInstances()` nur für neue Indizes `[oldCount, count)`, alte Instanzen unberührt; Voll-Rebuild (`RebuildAll()`) nur bei Kapazitäts-Wachstum (Godot-4.7-Pitfall `multimesh_allocate_data` wischt alte Instanzen — selten durch Chunk-Verdopplung, O(log N) über Lebenszeit) oder geschrumpftem Bestand (Fallback); danach `VerifyOldInstancePreserved()`-Selbstcheck (nur visueller Lauf) + `LogUpdate()`-Zeile (headless prüfbar: sichtbar/Puffer, Update-Typ, Index-Range, X/Z-Min/Max, 1-MultiMesh-Nachweis).
- **`GrowCapacity(MultiMesh, int)`** — `private void` (Perf-Welle 2) — Wächst die Puffer-Kapazität (MultiMesh.InstanceCount) in Chunks (Verdopplung ab InitialInstanceCapacity); danach `VisibleInstanceCount = 0` (frischer zeroed Puffer würde sonst als 1x1x1-Boxen am Weltursprung rendern); Aufrufer muss danach Voll-Rebuild machen (Godot-4.7-Pitfall, siehe Klassendoku).
- **`AppendInstances(MultiMesh, IReadOnlyList<Building>, int)`** — `private void` (Perf-Welle 2) — Inkrementelles Anhängen: SetInstanceTransform/SetInstanceColor ausschliesslich für die NEUEN Indizes `[_renderedCount, count)`, dann VisibleInstanceCount = count; alte Instanzen bleiben unberührt (Engine lädt nur Dirty-Regionen, partieller Upload).
- **`RebuildAll(MultiMesh, IReadOnlyList<Building>, int)`** — `private void` (Perf-Welle 2) — Voll-Rebuild-Fallback: schreibt ALLE Instanzen `[0, count)` neu + VisibleInstanceCount = count; nur bei Kapazitäts-Wachstum oder geschrumpftem Bestand (sollte nicht vorkommen).
- **`ApplyInstance(MultiMesh, Building, int)`** — `private static void` (Perf-Welle 2) — Setzt Transform + Farbe EINER Instanz (deterministisch aus der Building-Id: StableHash → HeightFor/FootprintFor/ColorFor, MakeTransform).
- **`MakeTransform(Building, float, float)`** — `private static Transform3D` (Perf-Welle 2) — Transform einer Gebäude-Instanz: Skalierung (footprint, height, footprint) + Position sim X/Y → Godot X/Z, Unterkante auf y = 0.
- **`VerifyOldInstancePreserved(MultiMesh, Building)`** — `private static void` (Perf-Welle 2) — Selbstcheck nach inkrementellem Update: liest die Transform der ältesten Instanz (Index 0) zurück und vergleicht sie mit dem deterministisch erwarteten Wert (Beweis „nur neue Instanzen gesetzt"); bei Abweichung GD.PrintErr. **Headless-Pitfall:** Dummy-Renderer stubbt die Instanz-Readback-API (GetInstanceTransform = immer Identity, Setter No-Ops, mesh_storage.h Godot 4.7) ⇒ Selbstcheck wird im Headless-Lauf übersprungen (Nachweis dort über das Index-Range-Log), aktiv im visuellen Lauf (Vulkan/D3D12).
- **`LogUpdate(MultiMesh, IReadOnlyList<Building>, int, int, string, string)`** — `private void` (Perf-Welle 2) — Debug-Log je Update (headless prüfbar): `N Gebäude in 1 MultiMesh (sichtbar/Puffer, 0 einzelne MeshInstance3D-Knoten) — Draw-Call-Schätzung ~1; Update-Typ: inkrementell (+N, nur Indizes [a,b) gesetzt, alte Instanzen unberührt) / Voll-Rebuild (Grund); X [min..max], Z [min..max] (Kartenbereich um 1000,1000)`.
- **`CreateMultiMesh()`** — `private static MultiMesh` — Ein Einheits-BoxMesh (1x1x1) mit UseColors + Fassaden-Material; TransformFormat Transform3D; InstanceCount bleibt zunächst 0 (Kapazität 0 — der erste Refresh legt die Start-Kapazität per GrowCapacity an).
...
- **`CityGrowthSystem`** — `public sealed class CityGrowthSystem(ulong seed)` — Automatisches Stadt-Wachstum entlang Infrastruktur (GDD 4, M3); M6.7-PERF: IsOccupied über Spatial-Hash (O(1)-Nachbarschaft statt O(N) linear — AdvanceTick 309→3020 ms → ~44 ms/Tick Release, determinismus-identisch); 2026-08-09 (FIX/BALANCE): BuildingSetback 10 m (Stop-Exklusivität) + Wachstumsbudget 0-2/Tick + Prototyp-Deckel 2000 (12.850 → 293 Gebäude in 300 Ticks).
- **Wachstums-Parameter** — Basisrate 1 %, Stop-Bonus x2 (Radius 12), Spacing 1.0, **Setback 10.0** (seit 2026-08-09; vorher 1.5 — Formel: Straßenhalbbreite 3.0 + halber größter Fußabdruck 7.0), MinSpacing 1.0, **CheckRadius 10.5** (vorher 2.5 — muss knapp > Setback sein, sonst verwirft die Fix32-Projektions-Rundung Kandidaten).
- **BALANCE-Parameter (2026-08-09)** — `MaxBuildingsPerTick = 2` (Wachstumsbudget 0-2/Tick, deterministisch via `_rng.Next(3)`, Erwartungswert 1.0, unabhängig von der Netzgröße — vorher ~400 Gebäude/Tick auf großem Netz); `MaxTotalBuildings = 2000` (Prototyp-Deckel Gesamtbestand, `Tick` kehrt früh zurück; später per Config, GDD 11); Zustand: `_growthBudgetThisTick`, `_buildingsThisTick`.
...
- **`Tick(Infrastructure, INetworkQuality)`** — `public void Tick(Infrastructure infrastructure, INetworkQuality network)` — Ein Wachstums-Tick (Segment-Abtastung); 2026-08-09: früher Return sobald `_buildings.Count >= MaxTotalBuildings` (Deckel), dann Würfeln des Wachstumsbudgets `_growthBudgetThisTick = _rng.Next(MaxBuildingsPerTick + 1)` + `_buildingsThisTick = 0`.
...
- **`SampleAlongSegment(Infrastructure, TransportSegment, Fix32)`** — `private void` — Kandidaten exakt BuildingSetback (10 m) seitlich der Trasse.
- **`TryGrowAt(Infrastructure, Position, Fix32)`** — `private void` — Wachstums-Check (Netz-Nähe, Belegt, Rate mit Stop-Bonus); 2026-08-09: Budget-Check `_buildingsThisTick < _growthBudgetThisTick` NACH dem RNG-Ziehen (RNG-Verbrauch je Kandidat bit-identisch — Determinismus/Lockstep unberührt), bei Platzierung `_buildingsThisTick++`.
...
### src/OpenMotion.Core.Tests/CityGrowthTests.cs (M6.7, +2 Spatial-Hash-Tests; 2026-08-09 +2 BALANCE/Setback-Tests)
- **`CityGrowthTests`** — xUnit: Infrastruktur-Datenmodell + automatisches Stadt-Wachstum (M3, GDD 3/4) — `GetSegmentsNear_ReturnsOnlySegmentsWithinRadius`, `GetSegmentsNear_EndpointDistance_IsRespected`, `AddStop_CanBeQueriedViaGetStopsNear`, `NoInfrastructure_NoGrowth`, `ZeroNetworkQuality_NoGrowth`, `RoadAndSeed_GrowsBuildingsBesideRoad_NotFarAway` (2026-08-09 angepasst: Setback-Bereich |Y| 1..2 → **9.5..10.5 m**, Netz-Radius-Assert 2.5 → **10.5 m**), `SameSeed_ProducesIdenticalBuildings`, `Growth_ProducesResidentialAndCommercial`, `StopProximity_AcceleratesGrowth`, `LowerNetworkQuality_SlowsGrowth` — alle unverändert grün (Spatial-Hash determinismus-identisch, keine Test-Anpassung nötig).
- **`Buildings_MinDistanceToRoadAxis_AtLeast4Meters`** — `[Fact]` (2026-08-09, +1): FIX-Nachweis — 5 Seeds (1/42/20260809/777/999), je Gebäude Abstand zur Straßenachse ≥ 4.0 m (tatsächlich 10.0 m Setback) + kein Gebäude auf einem Stop (Stops auf der Achse, dist² ≥ 16 — Fußgänger-/Infrastruktur-Exklusivität).
- **`Growth_After300Ticks_IsBoundedAndStillGrowing`** — `[Fact]` (2026-08-09, +1): BALANCE-Nachweis — 300 Ticks auf großem Netz (~40k Kandidaten/Tick) ⇒ `Assert.InRange(count, 1, 500)` (Stadt wächst weiter, aber begrenzt; real 293 statt ~12.000 vor der Drosselung; Test-Seed fix, kein Zufall zur Laufzeit).
- **`SpatialHash_LargeGrowthRun_CompletesFast`** — `[Fact]` (M6.7, angepasst 2026-08-09): PERF-Nachweis — 8 Strassen à 2500 m ⇒ 40.016 Kandidaten/Tick, 12 Ticks ⇒ ~480.000 IsOccupied-Checks in < 5 s (lineare Variante: Minuten-Bereich); Platzierungs-Assert unter BALANCE-Drosselung angepasst: ≥ 2000 → `InRange(1, 12*2)` (max. 2/Tick).
- **`SpatialHash_LargeScenario_SameSeedProducesIdenticalBuildings`** — `[Fact]` (M6.7, angepasst 2026-08-09): Determinismus-Nachweis auf dem Spatial-Hash-Pfad — 12 Ticks bei gleichem Seed bit-identisch (Id/Position/Typ/Kapazität/Wohlstand); Mengen-Assert ≥ 2000 → ≥ 1 (Wachstum findet statt, unter der Drosselung gedeckelt).
