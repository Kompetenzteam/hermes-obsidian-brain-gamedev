...
| CHANGELOG.md | Doku | geändert | Änderungshistorie (M1–M6.7); +3 neue Einträge: BALANCE (Wachstums-Drosselung 0-2/Tick, Deckel 2000), FIX (BuildingSetback 10 m, Stop-Exklusivität), PERF (MultiMesh inkrementell — nur neue Instanzen) — 221 Tests grün | — |
...
| KNOWN_ISSUES.md | Doku | geändert | Bekannte Probleme & offene Punkte; +3 neue: Export braucht vorherige Löschung der alten EXE, Headless-PerfMonitor-FPS nicht aussagekräftig, CityGrowth-Wachstum zu schnell (Balancing offen, M7) | — |
...
| scripts/BuildingVisualizer.cs | C# (Godot) | geändert | M6.6/M6.7-Gebäude-Rendering: EIN MultiMeshInstance3D (GPU-Instancing, Draw Calls ~1); Perf-Welle 2: INKREMENTELLES Update (nur neue Instanzen angehängt, Kapazität 4096→Verdopplung, Voll-Rebuild nur bei Kapazitäts-Wachstum/Schrumpf, Selbstcheck Instanz 0, Headless-Pitfall dokumentiert); Refresh rate-limitiert (max. 2 Updates/s) | — |
...
| src/OpenMotion.Core.Tests/CityGrowthTests.cs | C# (Test) | geändert | xUnit: Infrastructure, Stadt-Wachstum; M6.7: +2 NEU (Buildings_MinDistanceToRoadAxis_AtLeast4Meters, Growth_After300Ticks_IsBoundedAndStillGrowing), 2 Spatial-Hash-Tests + RoadAndSeed-Test an BALANCE/Setback angepasst — 221 Tests grün | — |
...
| src/OpenMotion.Core/City/CityGrowthSystem.cs | C# (Core) | geändert | Automatisches Stadt-Wachstum entlang Infrastruktur (GDD 4); M6.7-PERF: Spatial-Hash; FIX: BuildingSetback 1.5→10.0 m (Stop-Exklusivität), StopBonusRadius 5→12, GrowthCheckRadius 2.5→10.5; BALANCE: Wachstumsbudget 0-2/Tick (MaxBuildingsPerTick=2, deterministisch), Deckel MaxTotalBuildings=2000 — 12.850→293 Gebäude in 300 Ticks | — |
...
- **CHANGELOG.md** — Drei neue Einträge (Commit a3d8da5): (1) **BALANCE**: CityGrowth-Wachstum gedrosselt — deterministisches Budget `MaxBuildingsPerTick = 2` (0-2/Tick via `_rng.Next(3)`, Erwartungswert 1.0, Budget-Check NACH dem RNG-Ziehen ⇒ RNG-Verbrauch je Kandidat unverändert, Lockstep-deterministisch) + Prototyp-Deckel `MaxTotalBuildings = 2000` (früher Return in Tick); gemessen: 300 Ticks ⇒ **293 Gebäude statt ~12.000** (~1.0/Tick, Ziel 200-500); kleine Netze wachsen weiter (~0.2/Tick, Boni wirken). (2) **FIX**: Gebäude-Abstand zur Straßenachse ≥ 10 m — `BuildingSetback` 1.5 → 10.0 m (Formel: Straßenhalbbreite 3.0 + halber größter Fußabdruck 7.0), `GrowthCheckRadius` 2.5 → 10.5 m (MUSS > Setback sein, sonst Fix32-Rundung verwirft Kandidaten), `StopBonusRadius` 5 → 12 m (muss die 10 m seitlichen Gebäude erreichen); Nebeneffekt: Stop-Exklusivität (Stops auf Achse, Gebäude ≥ 10 m daneben). (3) **PERF (Perf-Welle 2)**: MultiMesh inkrementell — nur NEUE Instanzen `[renderedCount, count)` via `SetInstanceTransform`/`SetInstanceColor` + `VisibleInstanceCount` erhöhen, alte Instanzen unberührt (kein Voll-Rebuild, kein GPU-Voll-Upload je Wachstum); Puffer-Modell `InstanceCount` = Kapazität (Start 4096, Chunk-Verdopplung, 256 KiB) vs. `VisibleInstanceCount` = sichtbar; Godot-4.7-Pitfall (aus Engine-Quelltext verifiziert): InstanceCount-Wachstum = `multimesh_allocate_data` wischt alte Instanzen ⇒ selten (O(log N) über Lebenszeit); Selbstcheck: Instanz-0-Transform-Rücklese gegen deterministisch erwarteten Wert, im Headless-Lauf übersprungen (Dummy-Renderer stubbt Readback-API); Kamera-Nahbereichs-LOD bewusst NICHT umgesetzt (dokumentierte Entscheidung, kollidiert mit inkrementellem Anhängen).
- **KNOWN_ISSUES.md** — Commit 587a421, drei neue Punkte: (1) **Export-Neubau braucht vorherige Löschung der alten EXE** — Godot kann `build/openmotion_windows.exe` nicht überschreiben (PCK-Einbettung: „Temporäre Datei konnte nicht umbenannt werden") ⇒ vor jedem Export `rm -f build/openmotion_windows.exe`. (2) **Headless-PerfMonitor-FPS nicht aussagekräftig** — `Engine.GetFramesPerSecond` zeigt im Headless-Modus ~2 FPS (kein GPU-Rendering), Physics 0,2 ms kein Realmaß; echte FPS nur visuell (RTX). (3) **CityGrowth-Wachstum zu schnell (Balancing)** — ~12.850 Gebäude nach wenigen Sim-Sekunden (Sättigung), Wachstumsrate/Deckelung offenes Balancing-Thema (M7). [Hinweis: Punkt 3 wurde durch den BALANCE-Commit a3d8da5 (Budget 0-2/Tick, Deckel 2000) bereits adressiert — KNOWN_ISSUES selbst wurde im Fix-Commit nicht aktualisiert, offener Doku-Nachlauf.]
- **scripts/BuildingVisualizer.cs** — Perf-Welle 2 (Commit a3d8da5): Refresh() schreibt den MultiMesh-Instanz-Puffer NICHT mehr komplett neu. **Inkrementelles Anhängen** im Wachstumsfall (Normalfall): nur neue Indizes `[renderedCount, count)` via `AppendInstances()` (SetInstanceTransform/SetInstanceColor + VisibleInstanceCount = count), alte Instanzen unberührt — die Engine lädt nur Dirty-Regionen (partieller Upload). Neue private Methoden: `GrowCapacity()` (Kapazität = MultiMesh.InstanceCount, Start `InitialInstanceCapacity = 4096`, Chunk-Verdopplung; danach VisibleInstanceCount = 0, da frischer Puffer zeroed), `AppendInstances()`, `RebuildAll()` (Fallback: Kapazitäts-Wachstum wegen Godot-4.7-Pitfall `multimesh_allocate_data` [GPU-Puffer frei + CPU-Cache leer] oder geschrumpfter Bestand), `ApplyInstance()` (Transform + Farbe einer Instanz), `MakeTransform()` (sim X/Y → Godot X/Z, Unterkante y=0), `VerifyOldInstancePreserved()` (Selbstcheck: Instanz-0-Readback gegen erwarteten Wert; headless übersprungen — Dummy-Renderer stubbt `_multimesh_instance_get_transform` = Identity, Setter No-Ops; visuell Vulkan/D3D12 aktiv), `LogUpdate()` (je Update: sichtbar/Puffer, Update-Typ inkl. Index-Range-Nachweis, X/Z-Min/Max, 1-MultiMesh-Beweis). Kamera-Nahbereichs-LOD (~800 m) bewusst nicht umgesetzt (dokumentiert: Entfernen kollidiert mit inkrementellem Anhängen). API unverändert: `Initialize(CityGrowthSystem)`, `Refresh()`, `BuildingNodeCount` (= VisibleInstanceCount). Verifiziert: 221/221 Tests, headless `--quit-after 300` Exit 0, 0 Fehlerzeilen.
- **src/OpenMotion.Core/City/CityGrowthSystem.cs** — Commit a3d8da5, drei ineinandergreifende Änderungen: (1) **FIX Setback**: `BuildingSetback` 1.5 → **10.0 m** (3.0 m Straßenhalbbreite + 7.0 m halber größter Fußabdruck; Anforderung ≥ 4 m mit Abstand erfüllt), `GrowthCheckRadius` 2.5 → **10.5 m** (knapp > Setback, sonst verwirft die Fix32-Projektions-Rundung dist² = Setback²+ε > radius² alle Kandidaten), `StopBonusRadius` 5 → **12 m** (muss die 10 m seitlichen Gebäude erreichen); Invariante: Stops auf der Trassenachse ⇒ kein Gebäude auf einem Stop (Fußgänger-/Infrastruktur-Exklusivität). (2) **BALANCE Drosselung**: neues deterministisches Wachstumsbudget `MaxBuildingsPerTick = 2` — `Tick()` würfelt `_growthBudgetThisTick = _rng.Next(3)` (0-2, uniform, Erwartungswert 1.0, unabhängig von Netzgröße) und `TryGrowAt()` prüft `_buildingsThisTick < _growthBudgetThisTick` NACH dem RNG-Ziehen ⇒ RNG-Verbrauch je Kandidat bit-identisch (Determinismus/Lockstep unberührt); vorher: ~400 Gebäude/Tick auf großem Netz ⇒ ~12.850 in wenigen Sim-Sekunden (User-Feedback Ruckeln + unnatürlich). (3) **Prototyp-Deckel** `MaxTotalBuildings = 2000`: `Tick()` kehrt früh zurück, sobald erreicht (später per Config, GDD 11). Öffentliche API unverändert (Tick, GetBuildings, Seed, TickCount). Verifiziert: 221/221 Tests, 300 Ticks ⇒ 293 Gebäude.
- **src/OpenMotion.Core.Tests/CityGrowthTests.cs** — Commit a3d8da5: **+2 neue Tests** — `Buildings_MinDistanceToRoadAxis_AtLeast4Meters` (5 Seeds 1/42/20260809/777/999, je Gebäude Abstand zur Straßenachse ≥ 4.0 m + kein Gebäude auf einem Stop, dist² ≥ 16) und `Growth_After300Ticks_IsBoundedAndStillGrowing` (300 Ticks auf großem Netz, `Assert.InRange(count, 1, 500)` — Stadt wächst weiter, aber begrenzt; real 293). **3 angepasste Tests** (unter der Drosselung unerreichbare Asserts): `SpatialHash_LargeGrowthRun_CompletesFast` (≥ 2000 → `InRange(1, 12*2)`, Perf-Kern bleibt: ~480k IsOccupied-Checks in < 5 s), `SpatialHash_LargeScenario_SameSeedProducesIdenticalBuildings` (≥ 2000 → ≥ 1, Determinismus-Vergleich unverändert bit-identisch), `RoadAndSeed_GrowsBuildingsBesideRoad_NotFarAway` (Setback-Bereich |Y| 1..2 → 9.5..10.5 m, Netz-Radius-Assert 2.5 → 10.5 m). Full-Suite: **221/221 grün** (219 Bestand + 1 Setback-Test + 1 BALANCE-Test des Parallel-Agents).

### Neu (0)

- Keine neuen Dateien seit dem letzten Scan. (Commit a3d8da5 + 587a421 ändern nur 5 bestehende Dateien.)

### Entfernt (0)

- Keine Dateien seit dem letzten Scan entfernt. (assets/logo/logo_clean.ico war bereits im vorherigen Scan als entfernt markiert und ist weiterhin nicht im Arbeitsbaum.)
