...
| CHANGELOG.md | Doku | geändert | Änderungshistorie (M1–M6.7): M6.7-PERF-Einträge ergänzt (CityGrowth Spatial-Hash 309→3020 ms → ~44 ms/Tick, GPU-Instancing/Draw Calls ~1, Refresh-Throttling, PerfMonitor, 219 Tests) | — |
...
| KNOWN_ISSUES.md | Doku | geändert | Bekannte Probleme & offene Punkte; M6.7 aktualisiert: „extremes Ruckeln" als UMGESETZT markiert (GPU-Instancing), CityGrowth-Bottleneck als BEHOBEN markiert (Spatial-Hash), GPU-Testlage (RTX only), Headless-Timeout beim Export | — |
...
| project.godot | Konfiguration | geändert | Godot-Projekt: Main-Scene, Autoloads (LocalizationManager, SteamManager), 8 Kamera-Input-Actions, i18n DE/EN, GL Compatibility; M6.7-PERF: `run/max_fps=0` (Kommentar dokumentiert 60 — Diskrepanz), `msaa_3d=0`, `scaling_3d/scale=1.0`, `[physics] 60 Hz` (dokumentiert) | — |
...
| scripts/BuildingVisualizer.cs | C# (Godot) | geändert | M6.6/M6.7-Gebäude-Rendering: CityGrowth-Gebäude als Instanzen EINES MultiMeshInstance3D (GPU-Instancing, Draw Calls ~1 statt 1200+; Höhe/Farbe deterministisch aus Building-Id-Hash, Fassaden-Textur mit Fensterband), Refresh rate-limitiert (max. 2 Rebuilds/s, nur bei Anzahl-Änderung) | — |
...
| scripts/PerfMonitor.cs | C# (Godot) | neu | M6.7-Performance-Logging (V2-OPT Baseline): loggt alle 300 Sim-Ticks FPS, Time/Process/Physics/Navigation, Rendering-Objekte/-Primitive/-Draw-Calls + Gebäude-Anzahl (CityGrowth, nur Lesen); reines Logging, kein Sim-Eingriff | — |
| scripts/SimulationRunner.cs | C# (Godot) | geändert | SimLoop-Integration (M4→M6.7): 30-Hz-Tick-Akkumulator, Subsystem-Adapter, CityView + Demo-Linie + Fahrzeug-Visualisierung; M6.6: Referenzkarte einmal im Start-Setup (Wachstums-Infrastruktur), SetupBuildingVisualizer + Refresh-Hook, CityGrowthSubsystem.Growth-Exposé; M6.7: Refresh-Throttling (Vehicles 15 Hz / Buildings 2 Hz), PerfMonitor-Hook, LogPerformanceSettings | — |
...
| scripts/VehicleVisualizer.cs | C# (Godot) | geändert | M6.5/M6.7-Fahrzeug-Visualisierung: instanziiert Vehicle-Szenen je Typ, Linienfarbe aus 8-Spieler-Palette; M6.7-PERF: Transform-only-Refresh (kein Neu-Instanziieren), Knoten-Pool (ReconcileNodes), Routen-Distanz-Cache, TransformUpdateCount | — |
...
| src/OpenMotion.Core.Tests/CityGrowthTests.cs | C# (Test) | geändert | xUnit: Infrastructure, Stadt-Wachstum (Determinismus, Netz-Nähe); M6.7: +2 Spatial-Hash-Tests (SpatialHash_LargeGrowthRun_CompletesFast, SpatialHash_LargeScenario_SameSeedProducesIdenticalBuildings) | — |
...
| src/OpenMotion.Core/City/CityGrowthSystem.cs | C# (Core) | geändert | Automatisches Stadt-Wachstum entlang Infrastruktur (GDD 4); M6.7-PERF: IsOccupied über Spatial-Hash (O(1)-Nachbarschaft statt O(N) linear — 309→3020 ms → ~44 ms/Tick, determinismus-bit-identisch) | — |
...
- **CHANGELOG.md** — Zwei neue M6.7-PERF-Einträge: (1) CityGrowth-Kern auf Spatial-Hash (`IsOccupied` O(1)-Nachbarschaft statt O(N) linear — AdvanceTick 236 ms→2 s+ bei ~19k Kandidaten vorher, jetzt flach ~44 ms/Tick Release; Determinismus bit-identisch, alle 10 CityGrowthTests + Full-Suite 219/219 grün OHNE Änderung bestehender Tests; neue Tests `SpatialHash_LargeGrowthRun_CompletesFast` (40.016 Kandidaten/Tick, 4.507 Platzierungen, < 5 s) + `SpatialHash_LargeScenario_SameSeedProducesIdenticalBuildings`; ehrliche Einordnung: verbleibender Per-Tick-Anteil von GetSegmentsNear/GetStopsNear dominiert — linear in der Netzgröße, nicht mehr quadratisch in der Gebäudezahl), (2) GPU-Instancing (BuildingVisualizer: EIN MultiMeshInstance3D, Draw Calls ~1 statt 1200+), Refresh-Throttling (Vehicles 15 Hz / Buildings 2 Hz), max_fps 60 + MSAA aus (dokumentiert — Wert im Arbeitsbaum `run/max_fps=0`), PerfMonitor (NEU), Multithreading-Analyse (CityGrowth-Bottleneck identifiziert → Spatial-Hash-Fix, siehe KNOWN_ISSUES).
- **KNOWN_ISSUES.md** — M6.7-Update: „EXTREMES RUCKELN (User-Feedback)" als UMGESETZT markiert (GPU-Instancing/MultiMesh, Refresh-Throttling, max_fps 60 dokumentiert, PerfMonitor); „OFFEN — CityGrowth-Bottleneck" als ✅ BEHOBEN markiert (Spatial-Hash, Messung 309→3020 ms → ~44 ms/Tick Release, 219/219 Tests grün); GPU-Testlage (nur GeForce RTX lokal, AMD via Vulkan-Standard + CI-Software-Renderer Lavapipe, echte AMD-Messung erst M8); Headless-Timeout beim Export (>600 s, Prozess nicht vorzeitig killen).
- **project.godot** — M6.7-PERF: `[physics] common/physics_ticks_per_second=60` (bewusst Engine-Default — Godot-Physik und 30-Hz-Sim laufen getrennt, Physik > Sim für Rendering-Interpolation), `rendering/anti_aliasing/quality/msaa_3d=0` (Instancing-Boxen brauchen kein Multisampling), `rendering/scaling_3d/scale=1.0` (kein Downscaling), `application/run/max_fps=0` — **Diskrepanz im Projekt:** Kommentar + CHANGELOG dokumentieren „max_fps 60", der tatsächliche Wert ist 0 (unbegrenzt); faktenbasiert im Index abgebildet, Projektdatei nicht verändert.
- **scripts/BuildingVisualizer.cs** — M6.7-Perf-Umbau (GPU-Instancing): statt einzelner MeshInstance3D-Kinder je Gebäude jetzt EIN MultiMeshInstance3D `Buildings` (Einheits-BoxMesh 1×1×1, pro Instanz skaliert/positioniert, UseColors + Instanzfarbe = Basis-Ton × deterministische FNV-1a-Variante, 0.90+0.025·Variante); **Draw Calls ~1 unabhängig von N** (vorher 1200+ bei 600+ Gebäuden — User-Feedback „extremes Ruckeln"); Fensterband nicht mehr Geometrie, sondern prozedurale Fassaden-Textur (4×16 px, zur Laufzeit erzeugt, dunkles Band in 30,5 %..85,5 % Höhe; dokumentierte Vereinfachung: Band auch auf Dachfläche, BoxMesh-UVs nicht flächengetrennt); `Refresh()` baut den Instanz-Puffer NUR bei Änderung der Gebäude-Anzahl neu + intern zeitbasiert auf max. 2 Rebuilds/s limitiert (500 ms, robust gegen jede Aufruf-Frequenz); `BuildingNodeCount` = MultiMesh-Instanzen (Name aus Kompatibilität erhalten); Min/Max-Log der Gebäude-Positionen (Kartenbereich ~1000,1000, headless prüfbar). API: `Initialize(CityGrowthSystem)`, `Refresh()`, `BuildingNodeCount` unverändert — SimulationRunner-Integration bleibt kompatibel.
- **scripts/SimulationRunner.cs** — M6.7-PERF: Rendering-Refresh vom Sim-Tick entkoppelt — `_vehicleVisualizer?.Refresh()` alle 2 Sim-Ticks (15 Hz), `_buildingVisualizer?.Refresh()` alle 15 Sim-Ticks (2 Hz) mit Zählern (`_vehicleRefreshCount`/`_buildingRefreshCount`, `_startTimeMsec`); Sim selbst läuft unverändert 30 Hz (Hash-Sequenz identisch zu M6.6 — Rendering ist reines Lesen); PerfMonitor-Instanz in `_Ready()` + `LogSnapshot`-Hook alle 300 Ticks (Gebäude-Anzahl via `_cityGrowth.Growth.GetBuildings().Count`, nur Lesen) + Abschluss-Snapshot in `_ExitTree`; `LogPerformanceSettings()` loggt max_fps/msaa_3d/scale/physics_ticks_per_second als Headless-Nachweis; `_ExitTree`-Performance-Bericht (FPS headless ungecapped, Refresh-Raten x/s Soll ~15/~2, Transform-Update-Zahl).
- **scripts/VehicleVisualizer.cs** — M6.7-PERF: Refresh instanziiert KEINE Knoten mehr im Normalbetrieb — Transform-only-Update in EINER Zuweisung je Fahrzeug (`node.Transform = new Transform3D(new Basis(Vector3.Up, yaw), world)`), Knoten-Pool-Abgleich nur bei geänderter Fahrzeug-Anzahl (`ReconcileNodes()`, Dirty-Signal); neuer Routen-Distanz-Cache `_routeCache` (Linien-Id → kumulierte Haltestellen-Distanzen, nur bei Haltestellen-Anzahl-Änderung neu gebaut — spart sqrt/Allokation pro Refresh); `TransformUpdateCount` (long, headless-Perf-Nachweis); Log-Nachricht um Transform-Update-Zahl erweitert.
- **src/OpenMotion.Core/City/CityGrowthSystem.cs** — M6.7-PERF (Kern): `IsOccupied()` prüft statt ALLER Gebäude (O(N) je Kandidat ⇒ quadratisches AdvanceTick: 236 ms Tick 1 → 2 s+ Tick 12 bei ~19k Kandidaten) nur die **3×3-Nachbarzellen** eines Spatial-Hash — O(1)-Nachbarschaft. Zellgröße 2.0 Welt-Einheiten (2× Mindestabstand 1.0 ⇒ 3×3 deckt den gesamten Interaktionsradius dist < 1.0 ab); `Dictionary<long, List<Building>>` (long-Schlüssel aus gepackten Zellkoordinaten, floor-Division der Fix32-Raws, rein integer); Pflege: `AddToSpatialIndex` bei jeder Platzierung; `_buildings`-Liste bleibt für `GetBuildings()` (Entstehungsreihenfolge unverändert). **Determinismus bit-identisch:** Spatial-Hash ist reine Lookup-Struktur — keine Hash-Iteration, Zellen-Listen in Einfüge-Reihenfolge, Distanzvergleich unverändert (< MinSpacingSq) ⇒ gleiche bool-Rückgabe je Kandidat ⇒ RNG-Verbrauch unverändert ⇒ gleicher Seed ⇒ exakt dieselben Gebäude. Öffentliche API unverändert (Tick, GetBuildings, Seed, TickCount). Verifiziert: 219/219 Tests grün, Release ~44 ms/Tick flach (25–41 ms).
- **src/OpenMotion.Core.Tests/CityGrowthTests.cs** — M6.7: +2 Tests im neuen Abschnitt „Spatial-Hash Performance-Nachweis": `SpatialHash_LargeGrowthRun_CompletesFast` (8 Strassen à 2500 m ⇒ 40.016 Kandidaten/Tick, 12 Ticks, ≥ 2000 Platzierungen in < 5 s — lineare Variante: Minuten) und `SpatialHash_LargeScenario_SameSeedProducesIdenticalBuildings` (Determinismus auf dem Spatial-Hash-Pfad, 2000+ Gebäude bit-identisch); neuer Helfer `LargeRoadNetwork()`. Alle 10 bestehenden Tests unverändert grün.

### Neu (1)

- **scripts/PerfMonitor.cs** (M6.7, V2-OPT Baseline) — Performance-Logging-Knoten: loggt alle `LogIntervalTicks = 300` Sim-Ticks (10 s Simzeit bei 30 Hz) einen Snapshot der Engine-Performance-Monitore (Time/Fps, Time/Process, Time/PhysicsProcess, Time/NavigationProcess, Rendering-Objekte/-Primitive/-Draw-Calls) + `Engine.GetFramesPerSecond()` + Gebäude-Anzahl der Simulation (CityGrowth, nur Lesen). `LogSnapshot(int tick, int buildingCount)` wird vom SimulationRunner im bestehenden 300-Tick-Berichtsblock + beim Beenden (`_ExitTree`) gerufen; `SnapshotCount` als Verifikations-Zähler. Reines Logging — kein Sim-Eingriff, kein Determinismus-Einfluss; im Headless-Lauf liefern Rendering-Monitore 0 (Zeile dient als FPS-/Physics-/Process-Baseline).

### Entfernt (0)

- Keine Dateien seit dem letzten Scan entfernt. (Hinweis: Commit 2908d89 „chore: Temp-Artefakte aufgeräumt" löschte `.hermes/tmp_bv_verify.sh` und `perf_start.txt` — beide waren nie im Index; `assets/logo/logo_clean.ico` war bereits im letzten Scan als entfernt markiert.)
