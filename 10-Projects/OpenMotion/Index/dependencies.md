...
| City/CityGrowthSystem.cs | Infrastructure, TransportSegment, Position, INetworkQuality, Building, BuildingType, DeterministicRandom, Fix32; M6.7: Spatial-Hash (Dictionary<long, List<Building>>, System.Collections.Generic via implicit usings — keine neuen using); 2026-08-09: BuildingSetback 10.0 (vorher 1.5), StopBonusRadius 12 (vorher 5), GrowthCheckRadius 10.5 (vorher 2.5), Wachstumsbudget MaxBuildingsPerTick=2 + Deckel MaxTotalBuildings=2000 (Konstanten, keine neuen using) |
...
| scripts/BuildingVisualizer.cs | Godot (MultiMeshInstance3D, MultiMesh, BoxMesh, StandardMaterial3D, ImageTexture); OpenMotion.Core.City (CityGrowthSystem, Building, BuildingType, Position); Fix32 (Position.ToDouble in ToWorld); : Node3D; Kind der CityView (vom SimulationRunner instanziiert); M6.7: GPU-Instancing — EIN MultiMesh (Draw Calls ~1, unabhängig von N), Fassaden-Textur zur Laufzeit erzeugt; 2026-08-09 (Perf-Welle 2): inkrementelles Update — nur neue Indizes [renderedCount, count) via SetInstanceTransform/SetInstanceColor + VisibleInstanceCount, Kapazität 4096→Verdopplung (GrowCapacity, Godot-4.7-Pitfall multimesh_allocate_data), Selbstcheck GetInstanceTransform (headless übersprungen), OS.HasFeature (headless-Erkennung) |
...
| CityGrowthTests.cs | OpenMotion.Core.City, Xunit; M6.7: +2 Spatial-Hash-Tests (LargeRoadNetwork mit 8×2500-m-Strassen, Stopwatch-Performance-Nachweis); 2026-08-09: +2 neue Tests (Buildings_MinDistanceToRoadAxis_AtLeast4Meters — 5 Seeds + Stop-Exklusivität; Growth_After300Ticks_IsBoundedAndStillGrowing — BALANCE-Deckel), 3 Asserts an Setback 10.0/Budget 0-2 angepasst |
...
- **CityGrowthSystem.cs (2026-08-09, FIX/BALANCE)** → BuildingSetback 10.0 / GrowthCheckRadius 10.5 / StopBonusRadius 12 (Parameter-Triple muss konsistent bleiben: CheckRadius MUSS > Setback sein, StopBonusRadius MUSS die Setback-Gebäude erreichen — Änderung eines Werts erfordert Anpassung der anderen + der Setback-Tests); Wachstumsbudget 0-2/Tick (MaxBuildingsPerTick) + Deckel MaxTotalBuildings: Budget-Check sitzt NACH dem RNG-Ziehen ⇒ RNG-Verbrauch je Kandidat bit-identisch (Lockstep-NDD §3 unberührt, bestehende Hash-Baselines stabil); betroffen: CityGrowthTests (+2 neue, 3 angepasste), BuildingVisualizer (GetBuildings unverändert, nur Rendering), SimulationRunner (Growth-Exposé), CHANGELOG
- **BuildingVisualizer.cs (2026-08-09, Perf-Welle 2 inkrementell)** → SimulationRunner (Refresh alle 15 Sim-Ticks, 2 Hz — API unverändert Initialize/Refresh/BuildingNodeCount), CityGrowthSystem (GetBuildings, nur Lesen), CityView.tscn (Parent-Knoten); Puffer-Vertrag: InstanceCount = Kapazität (4096→Verdopplung), VisibleInstanceCount = sichtbar — Kapazitäts-Wachstum erzwingt Voll-Rebuild (Godot-4.7-Pitfall multimesh_allocate_data, dokumentiert); Selbstcheck (Instanz-0-Readback) nur im visuellen Lauf aktiv (Headless: Dummy-Renderer stubbt Readback-API); kein Sim-Einfluss (Hash-Sequenz identisch)
- **CityGrowthTests.cs (2026-08-09, +2 / -3 angepasst)** → CityGrowth-BALANCE-/Setback-Vertrag: Wachstum > 0 und gedeckelt (1..500 nach 300 Ticks), Gebäude-Mindestabstand ≥ 4.0 m zur Straßenachse + Stop-Exklusivität (dist² ≥ 16); Anpassung nötig, falls Budget/Deckel/Setback geändert werden; Perf-Kern (480k IsOccupied-Checks < 5 s) bleibt erhalten
- **CHANGELOG.md (2026-08-09, +3 Einträge)** → Doku über CityGrowthSystem (BALANCE/FIX), BuildingVisualizer (inkrementell), CityGrowthTests; keine Code-Abhängigkeit
- **KNOWN_ISSUES.md (2026-08-09, +3 Punkte)** → Doku: Export-Löschung nötig (build/openmotion_windows.exe vor jedem Export rm -f), Headless-PerfMonitor-FPS nicht aussagekräftig (~2 FPS ohne GPU), CityGrowth-Balancing offen (M7) — letzterer Punkt wurde durch Commit a3d8da5 (Budget 0-2/Tick, Deckel 2000) bereits adressiert, KNOWN_ISSUES selbst hat noch keinen Doku-Nachlauf
...
| **Stadt (M3/M6.7)** | Infrastructure.cs, Building.cs, CityGrowthSystem.cs | Gebaute Infrastruktur (Segmente/Haltestellen), automatisches Wachstum entlang der Wege (GDD 4); M6.7: Spatial-Hash für IsOccupied (O(1)-Nachbarschaft, determinismus-identisch) — AdvanceTick 309→3020 ms → ~44 ms/Tick Release; 2026-08-09: BuildingSetback 10 m (Gebäude nicht mehr auf 6-m-Strassen, Stop-Exklusivität), Wachstumsbudget 0-2/Tick + Deckel 2000 (12.850 → 293 Gebäude in 300 Ticks) |
| **Godot-Integration (M4→M6.7)** | scripts/SimulationRunner.cs, CameraController.cs, MapRenderer.cs, VehicleVisualizer.cs, BuildingVisualizer.cs, EnvironmentBuilder.cs, PerfMonitor.cs, scenes/*.tscn, VehicleColor.cs | 30-Hz-Tick-Akkumulator, deterministische Subsystem-Adapter, Referenzkarten-Rendering, Orbit-Kamera, Fahrzeug-Visualisierung, Gebäude-Visualisierung (M6.6), Umgebung (Boden/Himmel/Sonne, M6.6), prozedurale Fahrzeugmodelle; M6.7-PERF: GPU-Instancing (MultiMesh, Draw Calls ~1), Refresh-Throttling (15 Hz/2 Hz), PerfMonitor-Logging; 2026-08-09 (Perf-Welle 2): inkrementelles MultiMesh-Update (nur neue Instanzen, Kapazität 4096→Verdopplung, kein Voll-Rebuild je Wachstum) |
...
| **Tests** | src/OpenMotion.Core.Tests/*.cs (16 Suiten inkl. M5/M6/M6.7-Neu) | xUnit: Determinismus-, Roundtrip-, Netz- und Validierungsabdeckung (ADR-004/006); M6.7: +2 Spatial-Hash-Tests (Performance + Determinismus, 40.016 Kandidaten/Tick); 2026-08-09: +2 CityGrowth-Tests (Setback/Stop-Exklusivität, BALANCE-Deckel) — **221 Tests grün**; CI auf Windows+Linux |
